...
use aria_api::{
    ...
    CreatePortRequest, CreateQosPolicyRequest, CreateRouteTableRequest,
    CreateSecurityGroupRequest, CreateServiceRequest, CreateTenantRequest, HealthCheckListQuery,
    HealthCheckListResponse, HealthCheckResource, HealthCheckStatus, IpGroupListQuery,
    IpGroupListResponse, IpGroupResource, IpGroupStatus, MessageResponse, NetworkListQuery,
    NetworkListResponse, NetworkPolicyListQuery, NetworkPolicyListResponse, NetworkPolicyResource,
    NetworkPolicyStatus, NetworkResource, NetworkStatus, NodeCapability, NodeListQuery,
    NodeListResponse, NodeResource, NodeStatus, PlatformApiError, PortListQuery, PortListResponse,
    PortResource, PortStatus, QosPolicyListQuery, QosPolicyListResponse, QosPolicyResource,
    QosPolicySpec, QosPolicyStatus, ResourceCreateMetadata, ResourceMetadata,
    ...
    TenantStatus, UpdateBackendSetRequest, UpdateHealthCheckRequest, UpdateIpGroupRequest,
    UpdateNetworkPolicyRequest, UpdateNetworkRequest, UpdateNodeRequest, UpdatePortRequest,
    UpdateQosPolicyRequest, UpdateRouteTableRequest, UpdateSecurityGroupRequest,
...
fn qos_policy_status(rule_count: usize) -> QosPolicyStatus {
    QosPolicyStatus {
        phase: "ready".into(),
        rule_count,
    }
        ...
        .filter(|ip_group| {
            labels_match(&ip_group.metadata.labels, &selector)
                ...
                && optional_option_eq(
                    ...
                    ip_group.spec.network_id.as_deref(),
                )
    ...
    let items = store
        ...
        .filter(|policy| {
            labels_match(&policy.metadata.labels, &selector)
                ...
                && optional_option_eq(
                    query.network_id.as_deref(),
                    policy.spec.network_id.as_deref(),
                )
                && optional_eq(query.status.as_deref(), &policy.status.phase)
                && match query.action.as_deref() {
                    Some(action) => policy.spec.rules.iter().any(|rule| rule.action == action),
                    None => true,
                }
        })
        .collect::<Vec<_>>();
...
// --- QosPolicy (Phase 3.6: QoS platform migration) ---
