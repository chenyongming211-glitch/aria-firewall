...
| BUG-28 | Critical | agent/datapath | Controller 下发的 `NetworkPolicy` 端口过滤规则曾以 `bitmap_idx = None` / `is_new_port_set = false` 调用 `add_policy`，导致 `POLICY_TABLE` 写入 `has_port_filter = 1` 和反转后的默认 action，但 `PORT_BITMAP_POOL` 没有对应端口条目。端口 miss 后会把 allow+ports 变成全端口 drop，把 deny+ports 变成全端口 pass。现已在 agent compiler 为端口规则分配 deterministic bitmap，并在 materialize 时写入端口 bitmap。 | Fixed (2026-04-18) |
| BUG-29 | High | agent/datapath | Controller-written `NetworkPolicy` 清理路径曾只删除 `POLICY_TABLE`，没有删除对应 `PORT_BITMAP_POOL` 条目。现已在 compiled IR 中保存 `bitmap_idx` / 规范化端口表达式，并在 previous state 清理时同步删除端口 bitmap。 | Fixed (2026-04-18) |
| BUG-30 | High | controller/agent | 多条 `NetworkPolicy` 规则映射到同一个 `(network, src, dst, proto, direction)` eBPF policy key 时，后写规则会覆盖先写规则，controller 和 agent 均无冲突检测。现已在 controller 准入层拒绝重复/冲突 key，并在 agent compiler 对历史/旧状态做防御性跳过和 degraded report。 | Fixed (2026-04-18) |
| BUG-31 | Medium | controller/agent | `NetworkPolicy.rules[].ports` 曾缺少 controller 侧格式校验和 proto 约束；非法端口表达式或 proto=any/icmp 搭配 ports 会延迟到 agent apply 阶段失败。现已在 controller 准入层和 agent compiler 双重校验。 | Fixed (2026-04-18) |
| BUG-32 | Medium | agent/compiler | `stable_local_id` 将 `IpGroup` ID 映射到 u32 时没有碰撞检测；极低概率下不同 `IpGroup` 会共享 numeric ID，导致 ACL/LPM 语义错误。现已在 agent compiler 检测 numeric ID 碰撞，并跳过受影响对象。 | Fixed (2026-04-18) |
