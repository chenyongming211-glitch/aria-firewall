...
- agent 当前只实现 `shadow compile only`：会把 `Tenant / Network / Port / SecurityGroup / RouteTable` 编译成节点局部 `compiled state`，生成第一版本地 `reconcile plan` 与 `runtime plan (AttachPlan / MapPlan)` 并回报 `partial / failed` 结果，但尚未把这些结果 materialize 到 datapath
