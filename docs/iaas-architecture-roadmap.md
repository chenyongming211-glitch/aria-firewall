...
- agent 本地现已开始为 internal service listener 生成 `socket-selection-plan`，把 `lb_policy / session_affinity / forwarding_mode / local-backend-count / remote-backend-count` 收敛成第一版 node-local socket LB shadow selection 计划，作为后续 socket datapath 的前置输入
