...
- `RuntimeIntent / RuntimeExecutionSummary` 现已开始为 `services` 域单独保留 listener / socket-lb listener / packet-lb listener / backend member / forwarding projection 的细化摘要，并显式区分 `node_local / cross_node_native / cross_node_overlay / cross_node_hybrid` 等方向，作为后续 runtime apply 和 rollout 观察面的前置骨架
