...
- agent 当前会持久化 `desired-state-cache.json`、`compiled-node-state.json`、`reconcile-plan.json`、`runtime-plan.json`、`runtime-inventory.json`、`runtime-inventory-diff.json` 与 `runtime-intent.json`，作为 `desired state / compiled state / reconcile plan / runtime plan / runtime inventory / runtime inventory diff / runtime intent` 的本地恢复骨架
- 默认实现仍为内存态 backend
