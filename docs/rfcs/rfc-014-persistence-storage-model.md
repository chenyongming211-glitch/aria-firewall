...
- agent 当前会持久化 `desired-state-cache.json`、`compiled-node-state.json`、`reconcile-plan.json` 与 `runtime-plan.json`，作为 `desired state / compiled state / reconcile plan / runtime plan` 的本地恢复骨架
- 默认实现仍为内存态 backend
