...
use common::{
    ...
    DROP_ROUTE_MISS, DROP_SG_EGRESS, DROP_SG_INGRESS,
    FLAG_ACL_ON, FLAG_ANTI_SPOOF_PASSED, FLAG_CT_HIT, FLAG_IS_FORWARD,
    FLAG_LB_CT_AFFINITY, FLAG_LB_CT_ENABLED,
    ...
    TAP_ID_UNASSIGNED, TRACE_RESULT_DROP_IDENTITY, TRACE_RESULT_DROP_QOS, TRACE_RESULT_DROP_ROUTE,
    TRACE_RESULT_DROP_SECURITY, TRACE_RESULT_PASS, TRACE_TC_DROP, TRACE_TC_EGRESS,
    TRACE_TC_INGRESS, TRACE_XDP_DROP, XDP_DROP, XDP_PASS,
};
...
unsafe fn phase_post_accept_tc_ingress(
    ...
    if (p.flags & FLAG_PORT_RESOLVED) != 0 && (p.flags & FLAG_LB_HIT) == 0 {
        ...
        if (p.flags & FLAG_CT_HIT) == 0 && !sg::sg_check(p, info, SG_DIR_EGRESS) {
            load_packet_ids(info, p);
